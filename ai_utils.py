...
def predict_account(description: str, explanation: str, available_accounts: List[Dict]) -> List[Dict]:
    ...
    suggestions = vector_store.suggest_account(description, amount=0.0, limit=3)
    
    # Convert suggestions to expected format
    return [
        {
            'account_name': next((acc['name'] for acc in available_accounts 
                                if str(acc['id']) == sugg['account_id']), ''),
            'confidence': sugg['confidence'],
            'account': next((acc for acc in available_accounts 
                           if str(acc['id']) == sugg['account_id']), None),
            'reasoning': 'Based on vector similarity matching'
        }
...
def find_similar_transactions(transaction_description: str, transactions: list) -> list:
    vector_store = TransactionVectorStore()
    return vector_store.find_similar_transactions(
        description=transaction_description,
        user_id=str(transactions[0].user_id if transactions else None),
        limit=5
    )
